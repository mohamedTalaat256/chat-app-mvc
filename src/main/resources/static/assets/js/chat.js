    ...
    const onConnected = () => {
        ...
        (payload)=>{
            ...
            if(newMessage.senderId == selectedUserId){
                ...
                //notify
                 $.notify('<strong>'+newMessage.message+'</strong>',
                                {
                                    allow_dismiss: true,
                                    newest_on_top: true,
                                    delay: 5000,
        ...
        stompClient.subscribe('/user/'+userId+'/private-status',
                ...
                (payload)=>{
                    ...
                    $('#message_status_'+newMessage.id).removeClass("fa").addClass(["fas","text-primary"]);
                });
    }
    ...
    const renderMessageIn = (message) =>{
        var html = '';
               ...
               html +=          '</div>';
               html +=          '<div>';
               html +=              '<a href="#"  class="text-dark-75 text-hover-primary font-weight-bold font-size-h6"> '+$('#selected_user_name').html()+' </a>';
               html +=              '<span class=" font-size-sm"> '+dateFormatter(message.createdDate)+' </span>';
               html +=          '</div>';
               html +=     '</div>';
               html +=     '<div class="mt-2 rounded p-5 bg-light-primary text-dark font-weight-bold font-size-lg text-right max-w-400px message_item">';
    ...
    const renderMessageOut = (message) =>{
        ...
        html +=       '</div>';
        html +=    '</div>';
        html +=    '<div class="mt-2 rounded p-5 bg-light-success text-dark font-weight-bold font-size-lg text-left max-w-400px">';
     ...
     //   html +=  dateFormatter(message.createdDate);
        ...
        if(message.isRead == 0){
            html +=  '<i id="message_status_'+message.id+'" class="fa fa-regular flaticon2-check-mark icon-sm ml-2"></i>';
        }else{
            html +=  '<i id="message_status_'+message.id+'" class="fas text-primary flaticon2-check-mark icon-sm ml-2"></i>';
        }
       ...
       jQuery.ajax({
             ...
             success: function (data) {
                ...
                data.users.map((user)=>{
                    ...
                    html+= ' <div onclick="getChatMessages(' + user.id + ')" class="d-flex align-items-center justify-content-between mb-5">';
                    ...
                    html+= '   <a href="#"  class="text-dark-75 text-hover-primary font-weight-bold font-size-lg">'+user.name+'</a>';
                    html+= '    <span id="message_for_user_'+user.id+'" class="text-muted font-weight-bold font-size-sm">'+user.lastMessage+'</span>';
                   ...
                   html+= '    <span id="message_date_for_user_'+user.id+'" class="text-muted font-weight-bold font-size-sm">'+ dateFormatter(user.lastMessageDate)+'</span>';
                   if(user.unread !=0 ){
                        html+= ' <span class="label label-sm label-warning">'+user.unread+'</span>';
                   }
                    html+= ' </div>';
                    html+= ' </div>';
